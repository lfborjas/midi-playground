...
* [`src/clj/midi_playground/`](src/clj/midi_playground/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/midi_playground/`](src/cljs/midi_playground/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/midi_playground/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `midi-playground` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/midi_playground/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns midi-playground.example
  (:require [midi-playground.config :as config])
...
standalone `midi-playground.jar`.
...
```sh
port=2000 java -jar target/midi-playground.jar
```
